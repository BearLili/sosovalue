...
export class sosoValuRefferal {
  ...
  async getCodeVerification(email: string) {
    ...
    let client = null;
    const maxAttempts = 5;
    const maxImapRetries = 3;

    // 尝试建立IMAP连接，如果失败则重试
    for (let imapAttempt = 1; imapAttempt <= maxImapRetries; imapAttempt++) {
      try {
        client = await authorize(1); // 使用改进的authorize函数
        break; // 连接成功，跳出循环
      } catch (error) {
        logMessage(
          this.currentNum,
        ...
        if (imapAttempt === maxImapRetries) {
          logMessage(
            this.currentNum,
            this.total,
            "Failed to establish IMAP connection after all retries",
            "error"
          );
          return null;
        }
    ...
    if (!client) {
      logMessage(this.currentNum, this.total, "Failed to establish IMAP connection", "error");
      return null;
    }
    ...
    try {
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          // 检查连接状态
          if (!client.usable) {
            logMessage(this.currentNum, this.total, "IMAP connection lost, reconnecting...", "warning");
            try {
              await client.close();
            } catch (closeErr) {
              // 忽略关闭错误
            }
            client = await authorize(3);
          }
          ...
          logMessage(
            this.currentNum,
            this.total,
            `Error fetching emails: ${(error as Error).message}`,
            "error"
          );
          
          // 如果是连接错误，尝试重新连接
          if ((error as Error).message.includes('disconnected') || 
              (error as Error).message.includes('ECONNRESET') ||
              (error as Error).message.includes('socket')) {
            logMessage(this.currentNum, this.total, "Connection error detected, will retry with new connection", "warning");
            try {
              await client.close();
            } catch (closeErr) {
              // 忽略关闭错误
            ...
            try {
              client = await authorize(3);
              logMessage(this.currentNum, this.total, "Successfully reconnected to IMAP", "success");
            } catch (reconnectError) {
              logMessage(this.currentNum, this.total, `Failed to reconnect: ${(reconnectError as Error).message}`, "error");
            }
          }
        }
      ...
      logMessage(
        this.currentNum,
        this.total,
        "Error get code verification after all attempts.",
        "error"
      );
      return null;
    } finally {
      ...
      if (client) {
        try {
          await client.close();
          logMessage(this.currentNum, this.total, "IMAP connection closed", "success");
        } catch (closeError) {
          logMessage(this.currentNum, this.total, `Error closing IMAP connection: ${(closeError as Error).message}`, "warning");
        }
      }
    }
  }
